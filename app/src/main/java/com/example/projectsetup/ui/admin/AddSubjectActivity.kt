    ...
    private fun setListener() {
        btnAddSubject.setOnClickListener {
            ...
            if (::userId.isInitialized)
                adminActivityViewModel.addSubject(subjectName, userId)
        }
    }

    private fun observeData() {
        adminActivityViewModel.addSubjectLiveData.observe(this, Observer {
            when(it.status){
                Status.LOADING->{
                    progressModal.show()
                }
                Status.SUCCESS->{
                    ...
                    it.data?.let { sub->
                        showSuccessToast("${sub.name} subject added successfully")
                    }
                }
                Status.ERROR->{
                    progressModal.dismiss()
