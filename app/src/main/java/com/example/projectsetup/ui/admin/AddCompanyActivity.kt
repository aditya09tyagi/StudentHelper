...
class AddCompanyActivity : BaseActivity() {
    ...
    private fun setListener() {
        btnAddCompany.setOnClickListener {
            ...
            val companyWebsiteLink = etWebsite.text.toString()
            val companyLogoUrl = etLogo.text.toString()
            if (::userId.isInitialized)
                adminActivityViewModel.addCompany(
                    companyName,
                    companyWebsiteLink,
                    companyLogoUrl,
                    userId
    ...
    private fun observeData() {
        adminActivityViewModel.addCompanyLiveData.observe(this, Observer {
            when(it.status){
                Status.LOADING->{
                    progressModal.show()
                }
                Status.SUCCESS->{
                    ...
                    it.data?.let { cmp->
                        showSuccessToast("${cmp.companyName} company added successfully")
                    }
                }
                Status.ERROR->{
                    progressModal.dismiss()
